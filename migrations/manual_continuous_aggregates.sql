-- TimescaleDB Continuous Aggregates Setup Script
-- This script must be run OUTSIDE of a transaction block
-- Run with: psql postgresql://orderbook:orderbook123@localhost:5433/orderbook_timeseries -f migrations/manual_continuous_aggregates.sql
...
CREATE INDEX IF NOT EXISTS idx_ohlc_1m_symbol_time ON ohlc_1m(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_5m_symbol_time ON ohlc_5m(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_15m_symbol_time ON ohlc_15m(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_30m_symbol_time ON ohlc_30m(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_1h_symbol_time ON ohlc_1h(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_4h_symbol_time ON ohlc_4h(symbol_id, open_time DESC);
CREATE INDEX IF NOT EXISTS idx_ohlc_1d_symbol_time ON ohlc_1d(symbol_id, open_time DESC);
...
BEGIN
    ...
    RAISE NOTICE '   - 7 OHLC timeframes: 1m, 5m, 15m, 30m, 1h, 4h, 1d';
